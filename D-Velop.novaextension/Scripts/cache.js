...
class Cache {
	static async fetch(name, url, options = null, force = false) {
		...
		if (!options.headers["User-Agent"]) options.headers["User-Agent"] = userAgent;
		if (cached && cached.cache) {
			if (cached.etag) options.headers["If-None-Match"] = cached.etag;
			if (cached.modified) options.headers["If-Modified-Since"] = cached.modified;
		}
		...
		if ((!force) && cached && cached.cache && cached.expires && now < cached.expires) {
		...
		writeCache(name, { modified: modified, etag: etag, cache: latest, expires: now + 3600 * 1000 });
